(function($) {
    $.fn.asHTML5VideoControls = function(skinFolder, parentInst, videoElem, hideControls, hidePlayButton, defaultVolume, fullscreenNativeControls, html5VideoNoDownload, skinImages) {
        ...
        $videoObj.get(0).volume = volume / 2 + .1;
        if ($videoObj.get(0).volume === volume / 2 + .1) {
...
(function($) {
    $.fn.ashtml5lightbox = function(options) {
        ...
        var ELEM_TYPE = 0, ELEM_HREF = 1, ELEM_TITLE = 2, ELEM_GROUP = 3, ELEM_WIDTH = 4, ELEM_HEIGHT = 5, ELEM_HREF_WEBM = 6, ELEM_HREF_OGG = 7, ELEM_THUMBNAIL = 8, ELEM_DESCRIPTION = 9, ELEM_DIV = 10, ELEM_ORIGINALWIDTH = 11, ELEM_ORIGINALHEIGHT = 12, ELEM_SOCIALMEDIA = 13, ELEM_WEBLINK = 14, ELEM_WEBLINKTARGET = 15, ELEM_WEBLINKTEXT = 16;
        ...
        inst.createMarkup = function() {
            ...
            if (inst.options.titlestyle == "left") inst.$elem.prepend(elemText); else if (inst.options.titlestyle == "outside") inst.$lightbox.append(elemText); else inst.$elem.append(elemText);
        ...
        inst.embedHTML5Video = function($container, src, autoplay, loopvideo, mutevideo) {
            ...
            if (!inst.options.nativehtml5controls && !inst.options.videohidecontrols) {
                ...
                $("video", $container).asHTML5VideoControls(inst.options.skinsfolder, inst, ".html5-lightbox-video", inst.options.videohidecontrols, false, inst.options.defaultvideovolume, inst.options.nativecontrolsonfullscreen, inst.options.nativecontrolsnodownload, null);
            }
...
(function($) {
    $.fn.amazingslider = function(options) {
        var ELEM_ID = 0, ELEM_SRC = 1, ELEM_TITLE = 2, ELEM_DESCRIPTION = 3, ELEM_LINK = 4, ELEM_TARGET = 5, ELEM_VIDEO = 6, ELEM_THUMBNAIL = 7, ELEM_LIGHTBOX = 8, ELEM_LIGHTBOXWIDTH = 9, ELEM_LIGHTBOXHEIGHT = 10, ELEM_WEBM = 11, ELEM_OGG = 12, ELEM_TEXTEFFECT = 13, ELEM_DURATION = 14, ELEM_BUTTON = 15, ELEM_VIDEOTYPE = 16, ELEM_ALT = 17, ELEM_EXTRA = 18;
        ...
        AmazingSlider.prototype = {
            ...
            readTags: function() {
                ...
                $(".amazingslider-slides", this.container).find("li").each(function() {
                    ...
                    if (img.length > 0) {
                        ...
                        if (buttonObj.length > 0) if (buttonObj.parent().is("a")) button = buttonObj.parent().clone().wrapAll("<div/>").parent().html(); else button = buttonObj.clone().wrapAll("<div/>").parent().html();
                        var video = [];
            ...
            embedHTML5Video: function($container, src, autoPlay, muted, playsinline) {
                ...
                $container.html("<div class='amazingslider-video-container-" + this.id + "' style='position:relative;display:block;width:100%;height:100%;'><video class='slider-inline-video' style='width:100%;height:100%;'" + (this.options.nativehtml5controls && !this.options.videohidecontrols ? " controls='controls'" : "") + (this.options.nativecontrolsnodownload ? " controlsList='nodownload'" : "") + (muted ? " muted" : "") + (playsinline ? " playsinline" : "") + "></div>");
                $("video", $container).get(0).setAttribute("src", src);
                if (autoPlay) $("video", $container).get(0).play();
                if (!this.options.nativehtml5controls && !this.options.videohidecontrols) {
                    $("video", $container).data("src", src);
                    $("video", $container).asHTML5VideoControls(this.options.skinsfolder, this, ".slider-inline-video", this.options.videohidecontrols, this.options.videohideplaybutton, muted ? 0 : this.options.defaultvideovolume, this.options.nativecontrolsonfullscreen, this.options.nativecontrolsnodownload, null);
                }
                var instance = this;
                $("video", $container).off("ended").on("ended", function() {
                    instance.videoPaused = false;
            ...
            playVimeoVideo: function(href, autoplay, muted, playsinline) {
                ...
                if (this.options.previewmode) {
                    ...
                    var src = href + (href.indexOf("?") < 0 ? "?" : "&") + "autoplay=" + (autoplay && !ASPlatforms.isAndroid() ? "1" : "0") + (muted ? "&muted=1" : "") + (playsinline ? "&playsinline=1" : "") + "&api=1&player_id=amazingslider_vimeo_" + this.id;
            ...
            resizeSlider: function() {
                ...
                if (this.options.navmultirows && this.options.navdirection == "horizontal" && this.options.navthumbresponsivemode == "samecolumn") {
                    ...
                    for (var i = 1; i < $bullets.length; i++) if ((i + 1) % this.options.navthumbcolumn == 0) $bullets.eq(i).css({
                        "margin-right": 0
            ...
            createNav: function() {
                ...
                if (this.options.navstyle != "none") {
                    ...
                    if (instance.options.navmultirows && instance.options.navdirection == "horizontal" && instance.options.navthumbresponsivemode == "samecolumn") {
                        ...
                        for (var i = 1; i < $bullets.length; i++) if ((i + 1) % instance.options.navthumbcolumn == 0) $bullets.eq(i).css({
                            "margin-right": 0
                        }); else $bullets.eq(i).css({
                    ...
                    if (instance.options.navshowpreview) {
                        ...
                        $(imgLoader).on("load", function() {
                            ...
                            if (instance.options.navdirection == "vertical") {
                                ...
                                var $curImg = $("<div class='amazingslider-nav-preview-img-" + instance.id + "' style='display:block;position:absolute;overflow:hidden;width:" + instance.options.navpreviewwidth + "px;height:" + instance.options.navpreviewheight + "px;left:" + instance.options.navpreviewheight + "px;top:0px;'><img class='amazingslider-preview-elem' alt='" + instance.elemArray[bulletIndex][ELEM_ALT] + "' src='" + instance.elemArray[bulletIndex][ELEM_THUMBNAIL] + "' style='display:block;position:absolute;left:0px;top:0px;" + style + "' /></div>");
                                $previewImages.append($curImg);
                                if ($prevImg.length > 0) $prevImg.animate({
            ...
            showImage: function(slideDirection, transEffect) {
                ...
                $(imgLoader).on("load", function() {
                    ...
                    imgCurCodes += "<img " + instance.elemArray[instance.curElem][ELEM_EXTRA] + " alt='" + instance.elemArray[instance.curElem][ELEM_ALT] + "' class='amazingsliderimg amazingslider-img-elem amazingslider-img-elem-" + instance.id + "' data-originalwidth='" + this.width + "' data-originalheight='" + this.height + "' style='position:absolute;" + (ASPlatforms.isIE678() ? "opacity:inherit;filter:inherit;" : "");
        ...
        this.each(function() {
            ...
            this.options.linktitle = (Math.random() > .4 ? asbts("82,101,115,112,111,110,115,105,118,101,32") : "") + asbts("106,81,117,101,114,121,32,83,108,105,100,101,114");
            var instance = this;
            $.each($(this).data(), function(key, value) {
